...
archiving() {
    cd ..
    tar -cf "attendance_tracker_${input}_archive.tar" "attendance_tracker_$input"
    rm -rf "attendance_tracker_$input"
    echo "Attendance tracker interrupted. Everything is archived ===="
    exit 1
} 

            #Creating file generations
...
cd attendance_tracker_$input
        ...
        #Creating attendance_Checker.py
...
do
        case "$threshold" in
                "YES")
                        ...
                        ;;
                "NO")
                        echo " All thresholds remain as  default"
                        break
                        ;;
        esac
done
